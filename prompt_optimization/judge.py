...
judge_config = LMConfig(
    "openai/glm-4.6", temperature=0.75, max_tokens=128000, cache=True
)
...
class JudgeSignature(dspy.Signature):
    ...
    component_name: str = dspy.InputField(
        description="Name of the component being evaluated (e.g., planner._predictor.predict)"
    )
    component_trace: str = dspy.InputField(
        description="Structured trace of component execution (dict/JSON)"
    )
    prediction_trace: str = dspy.InputField(
        description="Full execution tree trace covering entire prediction"
    )
    component_feedback: str = dspy.OutputField(
        description="Actionable feedback for improving the component"
    )
...
class ComponentJudge:
    ...
    def __init__(
        self, *, prompt: str = GRADER_PROMPT, lm_config: LMConfig = judge_config
    ):
        """
        ...
        with dspy.context(lm=self.lm):
            self.predictor = dspy.ChainOfThought(
                JudgeSignature, instructions=self.prompt
            )

    def __call__(
        self, component_name: str, component_trace: dict, prediction_trace: str = ""
    ) -> str:
        """
