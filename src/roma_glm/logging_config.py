...
Example:
    Basic setup:
    >>> from roma_glm.logging_config import configure_logging
    >>> configure_logging(level="DEBUG", log_dir="/path/to/logs")

    Context binding:
    >>> from roma_glm.logging_config import set_execution_context
...
def configure_logging(
    ...
    if log_dir:
        ...
        log_file = log_dir / "roma_glm_{time:YYYY-MM-DD}.log"
...
def configure_for_development():
    ...
    configure_logging(
        ...
        colorize=True,
        log_dir=Path.home() / ".roma_glm" / "logs"
    )
