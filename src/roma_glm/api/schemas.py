...
class AgentConfigRequest(BaseModel):
    ...
    risk_level: Optional[str] = Field(
        default=None, description="Risk level: conservative, moderate, aggressive"
    )
    monitor_symbols: Optional[List[str]] = Field(
        default=None, description="Symbols to monitor"
    )
    price_alert_threshold: Optional[float] = Field(
        default=None, description="Price change threshold for alerts (%)"
    )
    rebalance_interval_seconds: Optional[int] = Field(
        default=None, description="Portfolio rebalancing interval in seconds"
    )


class ManualTradeRequest(BaseModel):
    """Request schema for manual trade execution."""

    symbol: str = Field(..., description="Trading symbol")
    action: str = Field(..., description="Trade action: buy, sell")
    quantity: float = Field(..., gt=0, description="Trade quantity")
    price: Optional[float] = Field(default=None, description="Target price (optional)")
