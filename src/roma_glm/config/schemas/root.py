...
from roma_glm.config.schemas.base import RuntimeConfig
from roma_glm.config.schemas.agents import AgentsConfig
from roma_glm.config.schemas.resilience import ResilienceConfig
from roma_glm.config.schemas.agent_mapping import AgentMappingConfig
from roma_glm.config.schemas.storage import StorageConfig
from roma_glm.config.schemas.observability import ObservabilityConfig
from roma_glm.config.schemas.logging import LoggingConfig
...
class ROMAConfig:
    ...
    def validate_global_consistency(self):
        ...
        zai_models = [m for m in models if m.startswith("zai/")]
        other_models = [
            m for m in models
            if not m.startswith("openrouter/")
            and not m.startswith("zai/")
        ...
        if provider_count > 1:
            warnings.warn(
                ...
                f"Anthropic models: {anthropic_models}, z.ai models: {zai_models}, Other models: {other_models}",
                UserWarning
            )
