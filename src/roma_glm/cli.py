...
name="agent",
help="Autonomous crypto trading agent commands"
)
app.add_typer(agent_app, name="agent", help="Autonomous agent commands")
console = Console()
...
def solve(
    ...
    Examples:
        roma-glm solve "What is the capital of France?"
        roma-glm solve "Plan a 3-day trip to Barcelona" --verbose
        roma-glm solve "Calculate fibonacci(10)" --output json
        roma-glm solve "Build a web scraper" --config my_config.yaml
    """
    try:
        ...
        async def solve_with_signal_handling():
            ...
            from roma_glm.types import ExecutionStatus
            from roma_glm.types.checkpoint_types import CheckpointTrigger
...
def config(
    ...
    Examples:
        roma-glm config
        roma-glm config --profile high_quality
        roma-glm config --output json
    """
    try:
        from roma_glm.config.manager import ConfigManager
        ...
        if output_format == "json":
            ...
            resilience_tree.add(
                f"Circuit Breaker: {config.resilience.circuit_breaker.enabled}"
            )
...
def server_start(
    ...
    Examples:
        roma-glm server start --reload
        roma-glm server start --workers 4 --port 8000
    """
    try:
        import uvicorn
        ...
        console_err.print(
            "[bold red]Error:[/bold red] uvicorn not installed. Install with: pip install 'roma-glm[api]'"
        )
        raise typer.Exit(code=1)
    except Exception as e:
...
def server_health(
    ...
    Examples:
        roma-glm server health
        roma-glm server health --url http://localhost:8000
    """
    try:
        ...
        console_err.print(
            "[bold red]Error:[/bold red] httpx not installed. Install with: pip install 'roma-glm[api]'"
        )
        raise typer.Exit(code=1)
    except Exception as e:
        console_err.print(
...
def exec_create(
    ...
    Examples:
        roma-glm exec create "Research ML papers"
        roma-glm exec create "Plan a trip" --max-depth 3 --profile high_quality
    """
    try:
        payload = {
            "goal": task,
...
def exec_status(
    ...
    Examples:
        roma-glm exec status abc123
        roma-glm exec status abc123 --watch
    """
    try:
        import time
...
def checkpoint_list(
    ...
    Examples:
        roma-glm checkpoint list abc123
        roma-glm checkpoint list abc123 --limit 10
    """
    try:
        from rich.table import Table
...
def checkpoint_get(
    ...
    Examples:
        roma-glm checkpoint get checkpoint_xyz
    """
    try:
...
def checkpoint_restore(
    ...
    Examples:
        roma-glm checkpoint restore checkpoint_xyz
    """
    try:
        from roma_glm.core.engine.solve import RecursiveSolver
        from roma_glm.config.manager import ConfigManager
        ...
        if success:
            ...
            console.print("   Use 'roma-glm solve' to continue execution")
        else:
...
def checkpoint_delete(
    ...
    Examples:
        roma-glm checkpoint delete checkpoint_xyz
        roma-glm checkpoint delete checkpoint_xyz --yes
    """
    try:
        if not confirm:
...
def metrics(
    ...
    Examples:
        roma-glm metrics abc123
        roma-glm metrics abc123 --breakdown
    """
    try:
        from rich.table import Table
